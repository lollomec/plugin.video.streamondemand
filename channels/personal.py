...
def personal_channel(item):
    ...
    for scrapedtitle, scrapedurltype, scrapedurl, scrapedplot, scrapedthumbnail, fanart in matches:
        ...
        if pagina_actual == pagina_a_mostrar:
            ...
            if scrapedurltype == "":
                itemlist.append(Item(channel=item.channel, action="play", title=title, fulltitle=title, url=url,
                                     thumbnail=thumbnail, fanart=fanart, plot=plot, folder=False))
            else:
                itemlist.append(
                    Item(channel=item.channel, action="personal_channel", title=title, fulltitle=title, url=url,
                         thumbnail=thumbnail, fanart=fanart, plot=plot, viewmode="movie_with_plot", folder=True))

            contador = contador + 1
            if contador > maximo:
                itemlist.append(Item(channel=item.channel, action="personal_channel", title=">> Página siguiente",
                                     extra=str(pagina_actual + 1), viewmode="movie_with_plot", folder=True))
                break
