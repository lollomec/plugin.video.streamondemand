...
def mainlist(item):
    ...
    if 2 in estados:
        itemlist.insert(0, Item(channel=item.channel, action="clean_ready", title="Elimina download completati",
    ...
    if 1 in estados or 0 in estados:
        itemlist.insert(0, Item(channel=item.channel, action="download_all", title="Scarica tutto",
                                contentType = item.contentType, contentChannel=item.contentChannel, contentSerieName = item.contentSerieName))

    if len(itemlist):
        itemlist.insert(0, Item(channel=item.channel, action="clean_all", title="Elimina tutto",
                                contentType = item.contentType, contentChannel=item.contentChannel, contentSerieName = item.contentSerieName))

    if not item.contentType == "tvshow" and config.get_setting("browser", "descargas") == True:
        itemlist.insert(0, Item(channel=item.channel, action="browser", title="Versione file scaricati", url = DOWNLOAD_PATH))
    
    if not item.contentType == "tvshow":
        itemlist.insert(0, Item(channel=item.channel, action="settings", title="Download setup...", text_color = "blue"))
...
def menu(item):
    ...
    op = ["Download", "Elimina dalla lista", "Reiniziallizza download", "Scarica da..."]
    ...
    if config.get_setting("library_add", "descargas") == True: 
      if filetools.isfile(final_path):
        if item.contentType == "movie" and item.infoLabels["tmdb_id"]:
          library_item = Item(title="Scaricato: %s" % item.downloadFilename, channel= "descargas", action="findvideos", infoLabels=item.infoLabels, url=final_path)
...
def sort_method(item):
    ...
    match_list_idimas  = {"IT"   : ["IT", "ITA", "Italiano"],
                          "SUB"  : ["SubITA", "SUB"],
                          "ENG"  : ["EN", "ENG", "English"]}
...
def download_from_server(item):
    ...
    progreso = platformtools.dialog_progress("Download", "Tentativo con: %s" % item.server)        
    channel = __import__('channels.%s' % item.contentChannel, None, None, ["channels.%s" % item.contentChannel])
    if hasattr(channel, "play") and not item.play_menu:

        progreso.update(50, "Tentativo con: %s" % item.server, "Connettendo con %s..." % item.contentChannel)
        try:
...
def download_from_best_server(item):
    ...
    progreso = platformtools.dialog_progress("Download", "Lista dei server disponibili...")
...
def save_download_video(item):
    ...
    if not platformtools.dialog_yesno(config.get_localized_string(30101), "Iniziare il download adesso??"):
...
def save_download_movie(item):
    ...
    progreso = platformtools.dialog_progress("Download", "Info sui contenuti film")
    ...
    progreso.update(0, "Aggiunta item...")
...
def save_download_tvshow(item):
    ...
    progreso = platformtools.dialog_progress("Download", "Info sui contenuti serie")
    ...
    progreso.update(0, "Aggiunta episodi...", "connesso con %s..." % item.contentChannel)
